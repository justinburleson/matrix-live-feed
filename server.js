...
const color = params.get("color");  // e.g. %2300ffaa
const fs    = params.get("fs");     // e.g. 16px
const glow  = params.get("glow");   // e.g. 8px
...
// ========== Digital Rain Background ==========
(() => {
  const rainSpeed = parseFloat(params.get("rainSpeed") || "1");    // 0.5..2
  const density   = parseFloat(params.get("density")   || "0.9");  // 0.2..1.3
  ...
  function resize() {
    ...
    // start drops at random vertical positions
  ...
  function step() {
    ...
    for (let i = 0; i < cols; i++) {
      ...
      drops[i] = (y > h || Math.random() > resetChance) ? 0 : y + speed;
    }
...
// ========== Feed + Typewriter + Phosphor Decay ==========
...
// Ingest endpoint for Make (one POST per item; no batching)
